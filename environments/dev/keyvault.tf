...
# AI backend's KV identity — same rationale as tool_backend above. It resolves
# app-db-password (the tool's postgres-password secret, shared server/admin login)
# and, once issued, app-llm-api-key. Unconditional on deploy_ai_apps so the grant
# precedes any key_vault_secret_id validation at apply.
resource "azurerm_key_vault_access_policy" "ai_backend" {
  count = var.deploy_ai_apps ? 1 : 0

  key_vault_id = azurerm_key_vault.env.id
  tenant_id    = data.azurerm_client_config.current.tenant_id
  object_id    = module.ai_apps[0].backend_kv_identity_principal_id
