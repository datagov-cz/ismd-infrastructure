...
variable "ai_llm_endpoint_url" {
  description = <<-EOT
    ...
    For AZURE_OPENAI the app speaks the Responses API and sends the deployment name in
    the request body, so the URL carries no {model} and no api-version:
    https://<host>.openai.azure.com/openai/v1/responses
  EOT
  type        = string
  default     = ""
}
...
# Azure OpenAI (Foundry) — see ai_foundry.tf.

variable "deploy_ai_foundry" {
  description = "Create the Azure OpenAI account + deployment for this env. Pay-per-token: costs nothing until tokens are consumed."
  type        = bool
  default     = false
}

variable "ai_foundry_location" {
  description = "Region for the Azure OpenAI account. Not var.location — model and deployment-type availability is per region; check with `az cognitiveservices model list -l <region>`."
  type        = string
  default     = "swedencentral"
}

variable "ai_foundry_deployment_name" {
  description = "Deployment name. This is what ismd-ai sends as ai_llm_model for AZURE_OPENAI, not the underlying model id."
  type        = string
...
variable "ai_foundry_deployment_capacity" {
  description = "Quota in thousands of tokens per minute (TPM). The real spend ceiling once ai_llm_enabled is true — the app's daily token cap only meters output tokens."
  type        = number
  default     = 10
}
